...
import os
import logging
from utils.messaging import TWILIO_NUMBER, WA_SANDBOX_NUMBER
...
class ChannelManager:
    ...
    def get_response_phone(self, from_phone_clean: str) -> str:
        """Get the phone number to send responses to (handles test mode)"""
        test_phone = os.environ.get("DIGEST_TEST_PHONE")
        if test_phone and os.environ.get("TEST_MODE", "False").lower() == "true":
            logger.info(f"📱 TEST MODE: Responses will go to {test_phone} instead of {from_phone_clean}")
            return test_phone
        return from_phone_clean